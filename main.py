...
def add_birthday(args, book):
    if len(args) != 2:
        raise IndexError("Invalid number of arguments for 'add-birthday' command. Usage: add-birthday [name] [birthday]")
    name, birthday = args
...
def show_birthday(args, book):
    if len(args) != 1:
        raise IndexError("Invalid number of arguments for 'show-birthday' command. Usage: show-birthday [name]")
    name = args[0]
    record = book.find(name)
...
def main():
    ...
    while True:
        ...
        if command in ["close", "exit"]:
            ...
            try:
                if len(args) < 2:
                    raise IndexError("Invalid number of arguments for 'add' command. Usage: add [name] [phone]")
                name, *phones = args
                record = book.find(name)
                if record:
                    for phone in phones:
            ...
            try:
                if len(args) != 2:
                    raise IndexError("Invalid number of arguments for 'change' command. Usage: change [name] [new phone]")
                name, new_phone = args
                record = book.find(name)
                if record:
            ...
            if len(args) != 1:
                print("Invalid number of arguments for 'phone' command. Usage: phone [name]")
            else:
                name = args[0]
                record = book.find(name)
                if record:
